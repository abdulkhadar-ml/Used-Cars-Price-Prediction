...
def load_models():
    try:
        ...
        st.error("Model files not found. Please ensure model.pkl and scaler.pkl are in the correct directory.")
        return None, None
...
with st.sidebar:
    if logo_b64:
        st.markdown(
            ...
            f'<p style="margin:6px 0 0 0;font-weight:800;font-size:0.9rem;color:{"#E8B84B" if mode else "#1A1A2E"};">'
        ...
        st.markdown(
            ...
            '<div style="width:60px;height:60px;border-radius:50%;background:linear-gradient(135deg, #E8B84B 0%, #D4A43A 100%);margin:0 auto;display:flex;align-items:center;justify-content:center;">'
            '<span style="color:#0D0D14;font-weight:800;font-size:1.5rem;">RR</span>'
            '</div>'
            f'<p style="margin:6px 0 0 0;font-weight:800;font-size:0.9rem;color:{"#E8B84B" if mode else "#1A1A2E"};">'
            f'RideRepublic</p></div>',
            unsafe_allow_html=True
    ...
    st.markdown(f"<hr style='border:none;border-top:1px solid {'#252535' if mode else '#E8E2D9'};margin:0.4rem 0;'>", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size:0.65rem;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:{'#6666AA' if mode else '#999999'};margin:0.6rem 0 0.3rem 0;'>Car Details</p>", unsafe_allow_html=True)
    ...
    st.markdown(f"<hr style='border:none;border-top:1px solid {'#252535' if mode else '#E8E2D9'};margin:0.8rem 0 0.5rem 0;'>", unsafe_allow_html=True)
...
hero_logo_html = ""
if logo_b64:
    hero_logo_html = (f'<img src="data:image/png;base64,{logo_b64}" '
                      f'style="width:60px;height:60px;border-radius:50%;object-fit:cover;" />')
else:
    hero_logo_html = ('<div style="width:60px;height:60px;border-radius:50%;background:linear-gradient(135deg, #E8B84B 0%, #D4A43A 100%);display:flex;align-items:center;justify-content:center;">'
                      '<span style="color:#0D0D14;font-weight:800;font-size:1.5rem;">RR</span>'
                      '</div>')
...
with cta_col:
    if st.button("Open Analytics Dashboard"):
        try:
            ...
            st.warning("Analytics page not found. Please ensure analytics.py is in the pages folder.")
